...
export default function EditTransportForm({ transport, users, places, role }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
      ...
      setError(data.error || "Failed to update transport");
    }
  };
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Edit Transport</h1>
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Update the transport details below.</p>
      </div>
